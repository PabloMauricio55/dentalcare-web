...
import { AccountView, DocumentsView } from "./AccountDocumentsView";
...
export function PatientPortalDashboard() {
  ...
  const section: Section = pathname.endsWith("/citas") ? "appointments" : pathname.endsWith("/controles") ? "checkups" : pathname.endsWith("/tratamientos") ? "treatments" : pathname.endsWith("/presupuestos") ? "estimates" : pathname.endsWith("/estado-cuenta") ? "account" : pathname.endsWith("/documentos") ? "documents" : preview;
  const isSummary = section === "summary";
  const title = section === "appointments" ? "Mis citas" : section === "checkups" ? "Próximos controles" : section === "treatments" ? "Tratamientos" : section === "estimates" ? "Presupuestos" : section === "account" ? "Estado de cuenta" : section === "documents" ? "Documentos" : isSummary ? "Resumen" : "Próximamente";
  ...
  return <div className={styles.shell}>
    ...
    <aside className={`${styles.sidebar} ${menuOpen ? styles.sidebarOpen : ""}`} aria-label="Navegación del portal">
      ...
      <nav>{navItems.map(({ id, label, icon: Icon }) => id === "summary" || id === "appointments" || id === "checkups" || id === "treatments" || id === "estimates" || id === "account" || id === "documents" ? <Link key={id} href={id === "summary" ? "/portal" : id === "appointments" ? "/portal/citas" : id === "checkups" ? "/portal/controles" : id === "treatments" ? "/portal/tratamientos" : id === "estimates" ? "/portal/presupuestos" : id === "account" ? "/portal/estado-cuenta" : "/portal/documentos"} className={section === id ? styles.active : ""} aria-current={section === id ? "page" : undefined} onClick={() => setMenuOpen(false)}><Icon size={18} aria-hidden="true" /><span>{label}</span></Link> : <button key={id} type="button" className={section === id ? styles.active : ""} onClick={() => select(id)}><Icon size={18} aria-hidden="true" /><span>{label}</span><small>Próximamente</small></button>)}</nav>
...
function Appointments({ notify, store }: { notify: (message: string) => void; store: ReturnType<typeof usePatientAppointments> }) {
  ...
  const setConfirmed = (value = true) => { if (value) setConfirmedAppointmentId(appointment?.id ?? null); };
