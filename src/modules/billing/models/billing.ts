...
export type Payment = { id: string; patientId: string; date: string; kind: PaymentKind; amount: number; method: PaymentMethod; chargeId: string; receiptId: string; shiftId: string };
export type PlanStatus = "Vigente" | "Completado" | "Cancelado";
export type AgreementPlan = { id: string; patientId: string; chargeId: string; total: number; downPayment: number; installmentCount: number; startDate: string; status: PlanStatus };
...
export type Receipt = { id: string; number: string; patientId: string; date: string; concept: string; amount: number; method: PaymentMethod; status: "Emitido" | "Anulado"; sentTo: string; printCount: number };
...
export type CashShift = { id: string; openedBy: string; openedAt: string; openingAmount: number; closedAt: string; expectedAmount: number; countedAmount: number; difference: number; closingNote: string; status: ShiftStatus };
export type MovementKind = "Ingreso" | "Egreso";
export type CashMovement = { id: string; shiftId: string; time: string; kind: MovementKind; concept: string; amount: number; registeredBy: string };
export type CashSummary = { opening: number; patientCash: number; otherMethods: number; manualIn: number; manualOut: number; expected: number };
